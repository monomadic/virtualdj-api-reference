...
User-created banks live under `~/Library/Application Support/VirtualDJ/Sampler/` on macOS. The `.vdjsample` binary format itself is not yet mapped; see `docs/Application Internals.md`.
...
for f in "AUDIO FX" "FAMOUS" "INSTRUMENTS" "VIDEO & SCRATCH"; do
  cp -p "/Applications/VirtualDJ.app/Contents/Resources/$f.xml" "examples/Samplerbanks/Built-In/$f.xml"
done
```
