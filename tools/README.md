...
| `verbdb.py` | Verb record store API over `docs/vdjscript-verbs.json` (authoritative, hand-editable): `get`/`put`/`next-incomplete`/`stats`/`search`/`check`/`generate`, plus `bootstrap` (merge-safe seed from the index + coverage audit + tracker). Replaces the record-then-promote-to-three-docs cycle with one `put` + regenerate. `generate` writes the derived views under `docs/VDJScript/generated/` (alias index, surface index, coverage/gap report); `--check` gates them byte-exact. `check` validates schema, alias resolution, index coverage, and count freshness. | `just verb <cmd>`, `just verb-views`, `check` + `generate --check` in `just check` |
