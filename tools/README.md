...
| `verbdb.py` | Verb record store API over `docs/vdjscript-verbs.json` (authoritative, hand-editable): `get`/`put`/`next-incomplete`/`stats`/`search`/`check`, plus `bootstrap` (merge-safe seed from the index + coverage audit + tracker). Replaces the record-then-promote-to-three-docs cycle with one `put`. `check` validates schema, alias resolution, index coverage, and count freshness. | `just verb <cmd>`, `check` in `just check` |
