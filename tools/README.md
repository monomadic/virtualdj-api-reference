...
| `fxdb.py` | Query API over the swept native effects catalog `tests/fx-introspection-dump.json`: `get <effect>` (spelling-tolerant; bare `fxdb.py <effect>` works too), `search` (filters: `--min/max-sliders`, `--min/max-buttons`, `--has-slider=LABEL`, `--has-button=LABEL`, `--in-cycle`, `--name-only`, `--format=json`), `stats`, `check`. No `put` — effect structure is machine-derived; re-run the sweep to refresh. | `just fx <cmd>`, `check` in `just check` |
