...
| `element_summary.py` | The XML inventory, its docs, the reader vocabulary and the skin probe fixtures, joined per element | Nothing new — the counterpart to `verb_summary.py`. Adds the per-element attribute documentation gap the inventory cannot see: its `documented` flag covers the ELEMENT NAME only, so `<panel>` reads documented while 31 of the 61 attributes shipped skins write it are explained in no doc. A miss reports any live probe that already tested the name, so a refuted candidate answers with its evidence instead of a spelling suggestion. `just element <name>`, `--format=json`. |
