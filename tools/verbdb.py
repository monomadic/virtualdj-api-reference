...
GENDIR = ROOT / "docs" / "VDJScript" / "generated"

GEN_HEADER = (
    "<!-- GENERATED by tools/verbdb.py from docs/vdjscript-verbs.json. "
    "Do not hand-edit; run `just verb-views` to regenerate. -->\n"
)
...
# ---- generated views (store -> Markdown) ------------------------------------

def gen_aliases(store: dict) -> str:
    # alias -> canonical, from every record's aliases list and alias-tier records
    pairs: dict[str, str] = {}
    for name, rec in store.items():
        for al in rec.get("aliases", []):
            pairs.setdefault(al, name)
        if rec.get("tier") == "alias" and rec.get("canonical"):
            pairs.setdefault(name, rec["canonical"])
    lines = [
        GEN_HEADER,
        "# VDJScript Alias Index (generated)\n",
        "Alias to canonical verb, derived from the verb record store "
        "([../../vdjscript-verbs.json](../../vdjscript-verbs.json)). "
        f"{len(pairs)} aliases.\n",
        "| Alias | Canonical |",
        "| --- | --- |",
    ]
    for al in sorted(pairs):
        lines.append(f"| `{al}` | `{pairs[al]}` |")
    return "\n".join(lines) + "\n"


def gen_surfaces(store: dict) -> str:
    surf: dict[str, list[str]] = {}
    for name, rec in store.items():
        for s in rec.get("surfaces", []):
            surf.setdefault(s, []).append(name)
    lines = [
        GEN_HEADER,
        "# VDJScript Surface Index (generated)\n",
        "Verbs that declare each scripting surface, derived from the verb record "
        "store ([../../vdjscript-verbs.json](../../vdjscript-verbs.json)). Only "
        "curated records carry surface tags.\n",
    ]
    for s in sorted(surf):
        names = sorted(set(surf[s]))
        lines.append(f"## {s} ({len(names)})\n")
        lines.append(", ".join(f"`{n}`" for n in names) + "\n")
    return "\n".join(lines) + "\n"


def gen_coverage(store: dict) -> str:
    c = counts(store)
    active = sorted(r["name"] for r in store.values()
                    if r.get("needs_test") and not r.get("blocked")
                    and r.get("test_status") != "Pass")
    blocked = sorted(r["name"] for r in store.values() if r.get("blocked"))
    passed = sorted(r["name"] for r in store.values()
                    if r.get("test_status") == "Pass")
    lines = [
        GEN_HEADER,
        "# VDJScript Coverage & Gap Report (generated)\n",
        "Live status derived from the verb record store "
        "([../../vdjscript-verbs.json](../../vdjscript-verbs.json)). "
        "Names-only official parity stays owned by the coverage audit; this view "
        "is the record store's own accounting.\n",
        f"- Total records: **{c['total']}**",
        f"- Documented (has description): **{c['documented']}**",
        f"- Locally observed (`Pass`): **{c['tested_pass']}**",
        f"- Flagged `needs_test`: **{c['needs_test']}** "
        f"({c['blocked']} hardware-blocked, {len(active)} active)\n",
        "## By tier\n",
        "| Tier | Count |",
        "| --- | --- |",
    ]
    for tier, n in c["by_tier"].items():
        lines.append(f"| {tier} | {n} |")
    lines += ["", "## By test status\n", "| Status | Count |", "| --- | --- |"]
    for st, n in c["by_test_status"].items():
        lines.append(f"| {st} | {n} |")
    lines += ["", "## Active work queue (needs_test, not hardware-blocked)\n"]
    lines.append(", ".join(f"`{n}`" for n in active) if active else "_none_")
    lines += ["", "## Hardware-blocked queue\n"]
    lines.append(", ".join(f"`{n}`" for n in blocked) if blocked else "_none_")
    lines += ["", "## Locally observed (Pass)\n"]
    lines.append(", ".join(f"`{n}`" for n in passed) if passed else "_none_")
    return "\n".join(lines) + "\n"


GENERATED = {
    "aliases.md": gen_aliases,
    "surfaces.md": gen_surfaces,
    "coverage.md": gen_coverage,
}
...
def cmd_generate(args):
    ...
    stale = []
    for fname, fn in GENERATED.items():
        path = GENDIR / fname
        want = fn(store)
        if check:
    ...
    if check:
        if stale:
            print("verbdb generate --check FAILED (stale/missing): "
                  + ", ".join(stale) + "\n  run `just verb-views`")
            sys.exit(1)
        print(f"verbdb generate check passed: {len(GENERATED)} views current")
    else:
        print(f"generated {len(GENERATED)} views in docs/VDJScript/generated/")
