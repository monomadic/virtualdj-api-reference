...
# bundle. `onchange` is the one attribute in a <device> definition that holds
# script (AKAI APC64's AfterTouchVelocity setting).
VENDOR_CONTROLLERS = Path("vendor/controllers")
CONTROLLERS_MANIFEST = Path("tests/controllers-manifest.json")
...
def vendor_controller_files() -> list[tuple[Path, str]]:
    ...
    """
    if not VENDOR_CONTROLLERS.exists() or not CONTROLLERS_MANIFEST.exists():
        return []
    import hashlib
    manifest = json.load(open(CONTROLLERS_MANIFEST))
    out, bad = [], []
    for index, block in enumerate(manifest["blocks"]):
        for member in block["members"]:
            path = VENDOR_CONTROLLERS / f"block-{index:03d}" / member["name"]
            if not path.exists():
...
def from_factory(known: set[str]) -> list[dict]:
    ...
    for path, root in vendor_controller_files():
        ...
        for context, value in pairs:
            ...
            if not found:
                continue
            out.append({"script": script, "source": "factory", "origin": str(path),
...
def main() -> int:
    ...
    parser.add_argument("--vendor-check", action="store_true",
                        help="verify vendor/controllers against the manifest and stop")
    args = parser.parse_args()

    if args.vendor_check:
        files = vendor_controller_files()
        if not files:
            sys.exit("vendor/controllers is absent — run `just controllers-vendor`")
        print(f"vendor/controllers verified: {len(files)} files match "
              f"{CONTROLLERS_MANIFEST} (bundle "
              f"{json.load(open(CONTROLLERS_MANIFEST))['bundle_version']})")
        return 0

    known = set(json.load(open(VERB_TABLE))["verbs"])
    factory = from_factory(known)
    merged = merge(from_catalog(args.app, known) + from_builtins(known)
                   + from_mappers(known) + factory + from_wiki(known)
    ...
    if args.check:
        ...
        if stored["by_source"].get("factory") and not factory:
            sys.exit("script corpus check FAILED: the artifact carries factory-mapping "
                     "snippets but vendor/controllers is absent — run `just controllers-vendor`")
    ...
    json.dump({
        "summary": {
            ...
            "factory_bundle": (json.load(open(CONTROLLERS_MANIFEST))["bundle_version"]
