...
def main():
    ...
    parser.add_argument("--callers", metavar="SYMBOL", help="compact historical caller sites, including rejected scan candidates")
    parser.add_argument("--get", metavar="CASE")
    parser.add_argument("--group")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()
    if args.callers:
        from runtime_parser_branch_routes import caller_report
        print(json.dumps(caller_report(args.callers), indent=2))
        return
    if args.triage:
