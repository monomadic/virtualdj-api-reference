...
- `connect` has local skin evidence: [official Lite](../xml/Skins/Built-In/Lite/Lite.xml) uses `<button action="connect">`. Local testing on VirtualDJ `v2026-m b9336` confirmed action/query behavior for logged-in and logged-out states.
...
Date: 2026-06-01
VirtualDJ build: user-provided local result, build not recorded
Test asset: Local FX slot/stem slot setup; `xml/Pads/FX-SLOTS.xml` is the nearest repo fixture
