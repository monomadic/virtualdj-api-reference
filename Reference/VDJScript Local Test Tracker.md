...
Do not promote these into ordinary user-facing verb guidance until a row has a concrete VirtualDJ build, setup, observed result, and notes. Use [Reference - Hidden Button Editor Tests.xml](../Test/Pads/Reference%20-%20Hidden%20Button%20Editor%20Tests.xml) for the low-risk pad probes.

| Candidate | Evidence | Likely surface/context | Suggested minimal repro | VirtualDJ build | Hardware | Result | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `stem_volume` | `Built-in app resource`; `Binary compiled table`; no exact class. | Stem controls, pad text/actions, stems-ready deck. | Load a stems-ready track and the hidden taxonomy test page; compare Pads 1-3 and shift-pad readbacks for `stem_volume 'Vocal'` / `'Instru'` at 50% and 100%; compare against ordinary `stem 'vocal'` behavior and audible output. | TBD | None required | Untested | Language description says this is slider-style stem volume with max at 100%, unlike `stem`, which defaults at 50% and isolates above 50%. Later pass should try aggregate names `Melody`, `Rhythm`, and `MeloVocal`. |
...
| `get_pad_page_name` / `pad_page_insplit` / `pad_page_favorite` / `pad_page_split` | `Binary compiled table`; exact symbols for all; `pad_page_insplit` also has a language description. | Pad-page state, split/favorite page UI. | Load the hidden taxonomy test page in normal pad mode and any split/favorite pad-page context; log shift pads 4-7 and observe Pad 6 query/color. | TBD | None required | Untested | Good first pass for understanding split/favorite pad-page internals. |
...
| `setting_if_unchanged` | `Binary compiled table`; exact `onQuery`. | Settings/config change guards. | Build a harmless custom-button query around one known setting; compare `setting_if_unchanged` before changing the setting, after changing it, and after saving/restarting if applicable. | TBD | None required | Untested | Needs a careful, non-disruptive fixture before any settings mutation is recommended. |
...
| `all_decks` / `combine_query` | `Binary compiled table`; exact `onExecute`, `onQuery`, `onQueryBool`, and `onQueryText`. | Query grammar, multi-deck combinators. | Start in custom buttons only: log bare readbacks, then test with tiny harmless boolean expressions if bare readback is accepted. | TBD | None required | Untested | Potential grammar-level helpers; do not use in pad fixtures until syntax and side effects are understood. |
