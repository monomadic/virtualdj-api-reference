...
- **Record verb facts with `just verb put`, not by hand-editing tables.** A local-test result, confidence, or evidence note goes into the record once (`just verb put <name> test_status=Pass confidence=local_test evidence="…"`), then `just verb-views` regenerates the derived Markdown — instead of promoting into the tracker and topical docs by hand. `just verb next-incomplete` gives the next active (non-hardware-blocked) work item; `just verb stats` the breakdown; the generated `docs/VDJScript/generated/coverage.md` is the cheap status snapshot.
