...
- **Effect controls come from the FX catalog, not prose tables.** `just get-fx Echo` gives the full slider/button map (spelling-tolerant: `BeatGrid` resolves to `Beat Grid`); `just find-fx --has-button=quant`, `--min-sliders=6`, `--name-only`, `--format=json` answer the rest. Do not hand-transcribe the catalog into Markdown.
