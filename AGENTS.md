...
- **Look verbs up through the tooling, not the monolith.** `just verb <name>` is the lookup (bare name works; `just verb get <name>` is the long form, and it follows aliases and suggests near matches on a miss). `just verb search <term>` when you do not know the name. Open `VDJScript Verbs.md` only at the specific lines a search hit points to.
...
- **Effect controls come from `just fx`, not prose tables.** `just fx Echo` gives the full slider/button map (bare name works, spelling-tolerant); `just fx search --has-button=quant`, `--min-sliders=6`, `--name-only`, `--format=json` answer the rest. Do not hand-transcribe the catalog into Markdown.
