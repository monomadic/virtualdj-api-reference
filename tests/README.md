...
| `binary-verbs.json` | The **structured** verb list extracted from the app itself: mangled `ACTION_<name>` implementation classes ∪ action names documented in `languages.zip`. Membership proves a name is a real verb. NOT complete — aliases and variant spellings are legitimately absent, so absence is not a disproof. Written by `tools/extract_binary_verbs.py`; query with `just binary-verb <name>`. |
