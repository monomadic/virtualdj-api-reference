...
| `verb-table.json` | **Authoritative.** VirtualDJ's own verb table, extracted from a serialised record array in `__DATA,__data` (`{const char *name; uint32 id; uint32 flags;}`): 1,028 records, 61 alias groups. Membership proves a name is a verb and absence disproves it; records sharing an `id` are the same verb, `flags==1` marking the alias spelling. Written by `tools/extract_verb_table.py`; query with `just verb-table <name>`. |
