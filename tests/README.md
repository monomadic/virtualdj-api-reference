...
| `action-contracts.json` | **Structural contract per verb**, read from the surviving ACTION_ class RTTI: name string → `std::type_info` → vtable. Gives every one of the 955 distinct verbs its implementation class (a checked 955/955 bijection), base-class family (`toggle`/`slider`/plain), context mixins (`IActionPlugin`, `IActionSampler`), vtable length, and the override matrix — execute, bool-query, number-query, text-query — with slot meanings calibrated from HTTP-proven verbs, not hard-coded. Agreement with the Tier-1 sweep: 652/652 on query verbs, 181/186 on action-only (the 5 outliers are the menu-verb slot-7 lead). Tier-2 structural evidence: predicts contract, does not prove behavior. Written by `tools/extract_action_contracts.py`; query with `just verb-contract <name>`. |
