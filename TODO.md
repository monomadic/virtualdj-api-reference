...
`docs/VDJScript Verbs.md`** to produce it. That was correct when hand-authored prose was the
best evidence in the repo. It is backwards now: the verb table decides existence outright, and
four artifacts carry contract data the monolith has never held.
...
**Why this is worth doing — the one real reason.** The index is built from the *weaker* source.
Where the prose and the artifacts disagree, the prose currently wins, silently, and the index is
consumed by `lint_mappers.py` and `verbdb.py bootstrap`. There is no gate anywhere that would
catch it. That is a correctness defect, and it holds whether or not anything is ever
restructured.

**The reconciliation is the actual deliverable, not the rewrite.** Diffing the old output
against the new one is a one-time audit of every documented verb claim against the extracted
evidence, and each discrepancy is one of exactly two useful findings:

- a documented claim the artifacts contradict → a correction to record in place; or
- a curated fact with no artifact home → names a **missing store field**, which is the real
  output: the schema gap between what we know and what the store can hold.

Neither is discoverable any other way, and neither shows up again once the generator is
switched — so run the diff and record it even if the counts come out small.

Plan:

1. Rewrite the generator to build the index from the store + `verb-table.json` +
   `action-contracts.json` + `verb-return-types.json`. Keep the output schema **byte-identical**
   at first, so `lint_mappers.py` and `verbdb.py bootstrap` are unaffected — prove the inversion
   with a no-diff commit, then extend the schema separately.
2. Reconcile every discrepancy before switching, into the two buckets above. Do not silently
   take either side.
3. Add whatever store fields step 2 identifies.
