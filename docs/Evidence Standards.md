...
| `tools/extract_vdjscript_symbols.py` | **Stale on this build** — reports 0 `ACTION_*` classes, because the names now survive only as mangled strings and not as `nm`-visible symbols. Do not read its silence as absence. Point it at an unstripped build instead (`--app`, see below). |
| `tools/extract_action_catalog.py` | **Official, and offline.** The Button Editor's action descriptions from `Resources/languages.zip` are the same prose the verbs appendix publishes at virtualdj.com (checked verbatim for `auto_bpm_transition`), so 816 descriptions — 97 of them naming parameters, 18 promising more than one argument — are readable without the network. This is the only source that says what a parameter *means*; probes can only say a token is not nonsense. `just action-catalog --get <name>`, `--cross-check` against the probe artifacts. |
| `tools/extract_script_corpus.py` | **Vendor-authored snippets, one file.** 1,390 scripts: 269 quoted inside catalog descriptions (documentation) and 1,131 from shipped Built-In pad/skin/samplerbank/videoskin XML (usage). Only `Built-In` trees are read — `examples/Pads/Quarantine` and the repo's own skins are excluded, or our fixtures would masquerade as vendor evidence. Use as a regression corpus: every snippet is a form the vendor considers valid. `just script-corpus --verb <name>`. |
| `tools/extract_action_vtables.py` | **Names every vtable slot**, but only on an unstripped build. Bundle `18.0.9246` (app `8.5.8769`) is the last one: 321,571 `nm` symbols, 952 `vtable for ACTION_*`, 5,325 `ACTION_*::` methods, no DWARF. `18.0.9482` is already stripped, so the boundary is 9337–9482. Names recovered there are a **hypothesis generator** for the current build — `overridden_slots` agrees 950/952 across the two, but any single verb's contract still comes from the current-build RTTI graph. |
