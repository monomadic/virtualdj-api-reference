...
| `get_effect_slider_default` | Get effect slider default value. `Local test` (v2026-m b9482): returned `off` for real and empty sliders — not a usable default; capture defaults via `effect_slider_reset` then `get_effect_slider_text` | `get_effect_slider_default 1` |
