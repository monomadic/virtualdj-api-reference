...
| `get_effect_slider_default` | Get effect slider default. Two forms (`Local test` v2026-m b9482): `get_effect_slider_default '<effect>' <n>` returns a normalized 0-1 default as text (`0.5` for Backspin S1); `get_effect_slider_default <slot> <n> <fallback>` is the value-context form built-in skins use in `<slider frommiddle=>` (returned `off` when read as text) | `get_effect_slider_default 'Echo' 1` |
