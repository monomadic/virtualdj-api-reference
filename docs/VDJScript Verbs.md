...
| `get_effect_slider_default` | Get effect slider default value. Signature unresolved: built-in skins use `get_effect_slider_default 1 1 0.5` in `<slider frommiddle=>`; another source gives `[effect_name] [slider_number]`. A `1 1 0.5` call in a pad `name=` returned `off` (`Local test` v2026-m b9482) — likely a wrong-arg/context miss, not a defect. For a reliable default, `effect_slider_reset` then `get_effect_slider_text` | `get_effect_slider_default 1` |
