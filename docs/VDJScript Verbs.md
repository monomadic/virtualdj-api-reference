...
- Generic FX panels should use introspection helpers instead of hardcoded parameter maps: `effect_has_slider`, `get_effect_slider_label`, `get_effect_slider_text`, `get_effect_slider_default`, `effect_has_button`, and `get_effect_button_shortname`. Built-in skins use these helpers for deck FX, video FX, and transition controls. Local evidence is still partial: on VirtualDJ `v2026-m b9336`, the FX introspection fixture opened Flanger in deck FX slot 1 and the GUI showed `Strength 50%`, `Speed 8bt`, `Tone n/a`, `Feedback 50%`, and `LFO AMP 40%`; the helper return strings, counts, defaults, and button availability have not yet been logged.
