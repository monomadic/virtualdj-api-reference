...
- `Built-in skin`: `examples/Skins/Built-In/Remote/9x16P.xml`, `9x16T.xml`, `3x4T.xml`, `16x9T.xml`
...
- `Built-in skin`: `examples/Skins/Built-In/Remote/16x9P.xml`, `19x9P.xml`
...
- `Official`: VDJScript verbs appendix
- `Official`: `examples/Pads/Built-In/pads_keycue.xml`
...
- `Official`: VDJScript verbs appendix
- `Official`: `examples/Pads/Built-In/pads_stems.xml`
...
- `Official`: VDJScript verbs appendix
- `Official`: `examples/Pads/Built-In/pads_sampler_velocity.xml`
...
- **`all` means the SELECTED slot, not every slot** (`Local test`, 2026-09-03, fixture
  `sampler_slots_differ` — slots 1-5 hold different samples, 6-8 are empty). With slot 1
  selected, `get_sample_name all` returned `'Dystopia Breaks'` and `sampler_color all`
  `#4D94F6`, both slot 1's values; after `sampler_select 5` the same two queries returned
  `'SB_IR&D_RISERS_140_093'` and `white`, matching slot 5 exactly. So `all` follows the
  selection rather than aggregating. `sampler_loaded all` stayed `yes` throughout while
  `sampler_loaded 8` was `no`, which rules out "every slot" independently. This resolves the
  question left open on 2026-09-02, when no fixture could tell "any", "first" and "every" apart.
- `sampler_select` takes a **1-based slot number on execute** but answers a **normalized 0-1
  slider value on query**: `sampler_select 5` selects slot 5 and then reads back `1`, and
  `sampler_select 1` selects slot 1 and reads back `0`. Do not feed its query value back into
  it as a slot number.
- **`all` is a reserved tail token across the sampler family** — `Local test` (2026-09-02, shared-lexicon probe plus direct queries): 22 verbs treat `all` differently from an arbitrary token, including `sampler_loaded`, `sampler_color`, `sampler_select`, `sampler_position`, `sampler_play`/`_stop`/`_play_stop`/`_play_stutter`, `sampler_default`, `sampler_load_to_deck`, the four `get_sample_*` queries, `loop_load`, `loop_select`, `load_skin`, `load_pulse`, `effect_stems`, `effect_dock_gui` and `apply_audio_config`. **What it does is not established.** In every fixture tested `<verb> all` returned exactly the bare value while nonsense errored or read `no`, so the probe proves the parser reserves the word and nothing more; on `loop_load`, `loop_select`, `load_skin` and `load_pulse` it inverts — `all` returns `E_INVALIDARG` where an arbitrary token is accepted as a name, so there the word is reserved for a form taking different arguments. Establishing meaning needs a state where slots disagree (several sampler slots loaded with different values), which no current fixture builds.
- Follow-up (2026-09-03) narrows it. `all` is **not** sampler-specific vocabulary: it is a target keyword, and query context collapses it to the first member rather than aggregating — exactly as [`deck all`](VDJScript%20Grammar.md#deck-all-broadcasts-on-execute-and-collapses-to-one-deck-on-query) does. With slots 1-5 loaded and 6-8 empty, `sampler_loaded all` reads `yes` and `get_sample_name all` returns slot 1's name, both identical to bare. `active` is accepted alongside it on the sampler verbs (`sampler_loaded active` -> `yes`, `get_sample_name active` -> slot 1's name) while the deck-only targets are not (`master`, `left`, `right`, `opposite` all read `no`). Verbs the probe did *not* flag ignore the tail entirely — `get_bpm all`, `get_bpm active` and `get_bpm zzqqx` all return the same value. Whether the sampler `all` broadcasts on execute the way `deck all` does is untested; it would write to five loaded slots.
...
- `Official`: VDJScript verbs appendix
- `Official`: `examples/Pads/Built-In/pads_saved_loops.xml`
...
- `Official`: VDJScript verbs appendix
- `Official`: `examples/Pads/Built-In/pads_manual_loop.xml`
- `Official`: `examples/Pads/Built-In/pads_loop_roll.xml`
- `Official`: `examples/Pads/Built-In/pads_hotcues.xml`
...
| `get_effect_slider_default` | Get effect slider default (target + optional index + fallback → normalized 0-1). `'<effect>' <fallback>` reads it as text (`Local test`: `'Backspin' 1`→`0.5`; `Built-in skin`: Broadcast video skin uses `'active' 0.5`). `<slot> <index> <fallback>` is the deck-skin `<slider frommiddle=>` value form (returned `off` as text) | `get_effect_slider_default 'active' 0.5` |
...
- Generic FX panels should use introspection helpers instead of hardcoded parameter maps: `effect_has_slider`, `get_effect_slider_label`, `get_effect_slider_text`, `effect_has_button`, and `get_effect_button_shortname`. Built-in skins use these helpers for deck FX, video FX, and transition controls. `Local test` (VirtualDJ `v2026-m b9482`, deck FX slot 1, Backspin): `get_effect_name`, `get_effect_slider_count`/`get_effect_button_count`, `effect_has_slider`/`effect_has_button`, and `get_effect_slider_text` are reliable and match the GUI. `get_effect_slider_label_full`/`get_effect_slider_name` return the full label (`Strength`), while `get_effect_slider_label`/`get_effect_slider_shortname` return the short form (`STR`). `get_effect_slider_default` takes a target (slot number, effect name, or `'active'`), optional index, and a trailing fallback, returning a normalized 0-1 default: `'<effect>' <fallback>` reads it as text; `<slot> <index> <fallback>` is the value-context form skins use in `<slider frommiddle=>`. See the helper reliability table in [Effects Engines](Effects%20Engines.md#effect-introspection-and-dynamic-controls).
