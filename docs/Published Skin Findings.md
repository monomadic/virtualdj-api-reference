...
The XML element inventory (`just inventory`, queried with `just list-xml-elements`) surfaced two attribute typos in VirtualDJ's own shipped skins. The built-in skin XML under `examples/Skins/Built-In/` is a verbatim reference copy, so the typos are preserved as-is; they matter here as evidence about how tolerant the skin parser is to unknown attribute names.
