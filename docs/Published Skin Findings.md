...
3. Compare against `docs/VDJScript Verbs.md` and the current official VDJScript verbs appendix.
4. Search exact terms on virtualdj.com first, then broader web/code search if needed.
5. Add a row here with the status: `Official`, `Official forum`, `Community`, `Published skin`, `Built-in skin`, `Published pad page`, `Built-in pad page`, `Local test`, `Needs test`, or a combination.
...
The XML element inventory (`just inventory`, queried with `just list-skin-elements`) surfaced two attribute typos in VirtualDJ's own shipped skins. The built-in skin XML under `examples/Skins/Built-In/` is a verbatim reference copy, so the typos are preserved as-is; they matter here as evidence about how tolerant the skin parser is to unknown attribute names.
...
The misspelling is systematic in the Pro/Performance/Vertical family: those skins also declare matching color defines, e.g. `<define color="hightlight" value="#47494c"/>` at [examples/Skins/Built-In/Desktop/Performance.xml:185](../examples/Skins/Built-In/Desktop/Performance.xml), and reference them as both an attribute value and a plain `color=` value on other `<line>` elements. So the color *name* `hightlight` works regardless (define names are arbitrary strings); the open question is only the attribute name.
