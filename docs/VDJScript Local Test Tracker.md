...
| `effect_command` plugin commands | Command strings are plugin-specific; built-in BeatGrid UI provides evidence but not a universal command map. | Built-in BeatGrid plugin UI or a pad page targeting BeatGrid in slot 1. | Load [Reference - BeatGrid Command Test.xml](../tests/Pads/Reference%20-%20BeatGrid%20Command%20Test.xml), press LOAD BEATGRID and SHOW GUI, then compare `set`, `get`, and `cur` command pads against the visible BeatGrid UI and any slot/plugin-focus requirements. | TBD | None required | Untested | Built-in plugin UI XML uses these command strings; needs a recorded VirtualDJ result before promoting to `Local test`. Do not generalize beyond BeatGrid. |
