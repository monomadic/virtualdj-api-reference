...
No built-in skin in this repo's copies uses `<visual type="waveform">`; all use `<rhythmzone>` and `<scratchwave>`. Built-in desktop skins switch between the two displays with panels keyed to the `skinWaveformType` setting, e.g. `<panel name="horizontal_scratch_active" visibility="not setting skinWaveformType 0">` (`examples/Skins/Built-In/Lite/Lite.xml` ~line 884). Source: `Built-in skin`.
...
Trimmed real example (`examples/Skins/Built-In/Desktop/Pro.xml` lines 4698-4706):
...
`examples/Skins/Built-In/Lite/Lite.xml` lines 864-882 (trimmed indentation):
...
`examples/Skins/Built-In/Desktop/Pro.xml` lines 4724-4738:
...
`examples/Skins/Built-In/Desktop/Pro.xml` lines 1336-1350:
...
Source: `Built-in skin`. The SDK example uses `classdeck="left"` / `classdeck="right"` on its songpos defines to make per-deck template variants (`examples/Skins/SDK Example - Custom Browser Skin/skin.xml` lines 19-33). Source: `Published skin`.
...
Real example (`examples/Skins/Built-In/Desktop/Pro.xml` lines 1203-1207, inside a jogwheel define):
...
- **`<zoomed x="" y="" width="" height=""/>`** — despite the waveform-sounding name, every local instance (8 in 5 files, e.g. `examples/Skins/Built-In/Lite/Lite.xml` line 1380) is a child of `<browser>` and defines the browser's zoomed-view rectangle. Not a waveform element. Source: `Built-in skin`.
...
- Counts in this document were generated by regex scans over `examples/Skins/Built-In/**/*.xml` plus the SDK example skin on 2026-07-12.
