...
task in `TASKS.md`, which promotes Phase 0 with one addition: the record store is fronted by a
stable `just` data-API surface (`just get-verb / put-verb / list-verbs / next-incomplete-verb / verb-stats`), so the
...
- **Final phase — Retire the monolith.** When the last family is extracted, replace
  ...
  `INDEX.yml`, `AGENTS.md`, and `docs/README.md`, and make the coverage guarantee the
  checker's job.
